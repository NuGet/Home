...
However, a scenario that is missing is "exclude a package asset from a current project, but do not exclude it from the dependency when the current project is packed". This missing feature is more obvious when looking at a project/flow matrix (see 3rd line):
