...
Therefore, just like adding any other property, it will need modifications in all of them.
...
This seems like an important use-case that should be enabled.
...
Finally, MSBuild properties don't allow additional metadata.
By using an item, we can add metadata, such as `Packages`: `<NuGetAuditSuppress Include="https://example.com/CVE1" Packages="ExamplePackage1;ExamplePackage2" />`.
For package sources this would have been helpful to enable credentials to authenticated package sources.
For `NuGetAuditSuppress`, it could enable new future features more easily.
...
Is there a better name than "NuGetAuditSuppress"?
Taking cargo's lead, perhaps `NugetAuditIgnore`?
